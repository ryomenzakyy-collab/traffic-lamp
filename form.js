...
(event) {
const hasilNama = document.getElementById
("hasil-nama");
console.log(event);
...
inputKota.addEventListener("change", function
(event){
const hasilKota = document.getElementById
("hasil-kota");
console.log(event);
hasilKota.textContent = event.target.value;
localStorage.setItem("kota", event.target.value)});

//input pesan

inputPesan.addEventListener("input", function
    (event){
        const hasilPesan = document.getElementById
        ("hasil-pesan");
        console.log(event);
        hasilPesan.textContent = event.target.value;
        localStorage.setItem("pesan", event.target.value)})
